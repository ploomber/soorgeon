...
chart = (
    df.groupby(top_20_country)["Basic_Cost_Per_Month"]
    .sum()
    .sort_values(ascending=False)
    .plot(kind="bar", color="maroon")
)
chart.set_xticklabels(chart.get_xticklabels(), rotation=45)
...
plt.figure(figsize=(10, 5))
chart = (
    df.groupby(top_20_country)["Standard_Cost_Per_Month"]
    .sum()
    .sort_values(ascending=False)
    .plot(kind="bar", color="lightseagreen")
)
chart.set_xticklabels(chart.get_xticklabels(), rotation=45)
plt.title(
    "Top 20 of Country based on Total Standard Cost Per Month", fontsize=15, pad=12
)
plt.xlabel("Country", fontsize=12)
...
plt.figure(figsize=(10, 5))
chart = (
    df.groupby(top_20_country)["Premium_Cost_Per_Month"]
    .sum()
    .sort_values(ascending=False)
    .plot(kind="bar", color="peru")
)
chart.set_xticklabels(chart.get_xticklabels(), rotation=45)
plt.title(
    "Top 20 of Country based on Total Premium Cost Per Month", fontsize=15, pad=12
)
plt.xlabel("Country", fontsize=12)
